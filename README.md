# ```<header_tag>``` RTE
This is ```<header_tag>```'s main website - Currently in development.
This project's structure was created by using  __DevTips Starter Kit__, you can view it at [DevTipsStarterKit.com](http://devtipsstarterkit.com).
...
```
git clone https://github.com/mortl/headertagtest headertag
```
...
│   │   └───js -> Scripts
│   ├───sass  -> Sass styles
│   │   └───main.scss -> Main file in where all other sass files should be included. (I used SCSS instead of SASS)
│   ├───vendors -> Vendors folder for all the dependencies (Managed by Bower)
│   └───views -> Templates directory for HTML (Also works with Jade fileS) files
│   │   └───index.html
